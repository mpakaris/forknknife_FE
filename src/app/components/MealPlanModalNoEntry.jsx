  ...
  return (
    <div className={`bg-white shadow-lg flex rounded-lg border-2 ${isPast ? 'border-gray-300 opacity-50 cursor-not-allowed' : 'border-red-600'} h-48`}>
      ...
      <div className="flex flex-col justify-center p-4 w-2/3"> {/* Text section */}
        ...
        {isPast ? (
          <p className="text-md mb-2">No meal was chosen.</p>
        ) : (
          <>
            <p className="text-xs mb-2">No Lunch chosen. But there is still time.</p>
            <button className="bg-teal-800 text-white text-sm px-4 py-2 rounded">Find your Meal!</button>
          </>
        )}
      </div>
    </div>
